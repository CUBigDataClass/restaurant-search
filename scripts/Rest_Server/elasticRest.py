...
def reviews_query():
    ...
    res = es.search(index="mydb_reviews", body={"query":{"match":{"text":str(query_word)}},"aggs":{"by_business_id":{"terms":{"field":"business_id.keyword", "size":10000}}}})
    buckets = res['aggregations']['by_business_id']['buckets']
    business_ids = set()
    for bucket in buckets:
        business_ids.add(str(bucket["key"]))
   # res = es.search(index="mydb_reviews",body={"query":{"match":{"text":"taco"}}, "from": 0, "size": 10000})
   # hits = res['hits']['hits']
   # business_ids = set()
   # for el in hits:
   #     business_ids.add(str(el['_source']['business_id']))
    print("B1:",business_ids, flush=True)
    res1 = es.search(index="mydb_restaurants", body={"query":{"bool":{"must":{"match_all" : {}},"filter" :{"geo_distance":{"distance" : "10km","location" : location}}}},"from": 0, "size": 100})
    hits = res1["hits"]["hits"]
    business_ids2 = set()
    for el in hits:
        business_ids2.add(str(el["_source"]["business_id"]))
    print("B2:",business_ids2, flush=True)
    final_business_ids = business_ids.intersection(business_ids2)
    print("FB:",final_business_ids, flush=True)

    res2 = es.search(index="mydb_restaurants", body={"query":{"terms":{"business_id.keyword":list(final_business_ids)}},"from":0,"size":len(final_business_ids)})
    final_result = []
    hits = res2["hits"]["hits"]
